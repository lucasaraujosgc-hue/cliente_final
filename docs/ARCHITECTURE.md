...
| `nfse/` (pasta) | Emissor de NFS-e Nacional (Swaggers oficiais em `docs/nfse-nacional/01-api/`). `status` (gating), `cert` (PKCS#12 → agente mTLS + PEM, node-forge), `inscricao` (CNPJ **string** — alfanumérico NT-009), `config` (CRUD config/atividades — o contador informa **todos** os códigos), `dps` (XML da DPS; `NFSE_DPS_VERSAO`, IBS/CBS gated por `NFSE_IBSCBS_ENVIAR`), `validate` (validação estrutural pré-envio), `sign` (XMLDSig enveloped), `client` (HTTP mTLS: `POST /nfse`→201, `GET|HEAD /dps`, `GET /nfse`, eventos; params no ADN `/parametrizacao`), `params` (cache convênio+alíquota), `emitir` (julga 201 vs processando, persiste), `reconcile` (`processando`→`emitida`/`rejeitada`; rota `/sincronizar`), `distribuicao` (busca no ADN `/contribuintes/DFe/{NSU}` as NFS-e do prestador não emitidas por aqui → `nfse_emissoes` com `origem='distribuicao'`), `events` (cancelamento e101101), `nfseXml` (lê o XML da NFS-e), `danfse`/`danfseRender` (DANFSe **gerado local** do XML — API do ADN sobrestada, NT-008; pdf-lib+qrcode), `chave` (parse 50 pos.), `cnpjLookup` (BrasilAPI→ReceitaWS), `log` (JSON sem segredo) |
